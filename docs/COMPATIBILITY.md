...
| Claude Code / Codex, production OAuth | Streamable HTTP MCP + OAuth | Implemented and documented | External issuer + HTTPS deployment not yet verified |
...
| Claude web/Desktop/Cowork | Host-dependent MCP | No Ninai-specific proof | Not verified |
| ChatGPT | Host/plan/workspace-dependent | No Ninai-specific proof | Not verified |
| Local desktop control panel | Local PyWebView + SQLite | Implemented and tested | Runs only as local vault owner |
| Hosted control center | WSGI UI/API over PostgreSQL | Implemented and unit tested | Deployment integration pending |
...
The local real-host gate is recorded in [CROSS-PROVIDER-SMOKE-TEST.md](CROSS-PROVIDER-SMOKE-TEST.md), including post-revocation denial. Production OAuth/HTTPS deployment remains a separate launch gate, so the public site continues to say “hosted beta in development.”
