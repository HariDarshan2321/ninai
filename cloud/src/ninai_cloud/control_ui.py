...
.card{background:white;border:1px solid #dfe7e2;border-radius:12px;padding:18px;margin:12px 0}button{cursor:pointer}input,select{padding:7px;margin:3px}
...
async function createConnection(){let provider=prompt('Provider: anthropic or openai','anthropic');if(!provider)return;let client_type=provider==='openai'?'codex':'claude-code';let display_name=prompt('Connection name',client_type);if(!display_name)return;let created=await api('/connections',{method:'POST',body:JSON.stringify({provider,client_type,display_name})});if(created.personal_access_token)alert('Copy this token now. It will not be shown again:\n\n'+created.personal_access_token);await load()}
async function createWorkspace(){let name=prompt('Workspace name','My Ninai');if(name){let workspace=await api('/workspaces',{method:'POST',body:JSON.stringify({name})});alert('Workspace created: '+workspace.id+'\nObtain a token containing this workspace ID to continue.')}}
async function grantProject(id){let scope_id=prompt('Project ID to grant');if(!scope_id)return;await act('/connections/'+id+'/grants',{scope_kind:'project',scope_id,can_read:true,can_propose:true,can_auto_activate:false})}
...
async function load(){try{let [o,m,c,a,p]=await Promise.all([api('/overview'),api('/memories?status=proposed'),api('/connections'),api('/activity'),api('/projects')]);
...
<section class="card"><h2>Projects</h2><button onclick="createProject()">New project</button>${p.items.map(x=>`<p><b>${esc(x.name)}</b> · <code>${esc(x.id)}</code></p>`).join('')||'<p class="muted">Create a project for scoped memory.</p>'}</section>
...
<section class="card"><h2>Connections</h2><button onclick="createConnection()">New provider connection</button>${c.items.map(x=>`<p><b>${esc(x.display_name)}</b> · ${esc(x.provider)} · ${esc(x.status)} ${x.status==='active'?`<button onclick="grantProject('${x.id}')">Grant project</button> <button onclick="testConnection('${x.id}')">Test setup</button> <button class="danger" onclick="act('/connections/${x.id}/revoke')">Revoke</button>`:''}</p>`).join('')||'<p class="muted">No connections.</p>'}</section>
<section class="card"><h2>Recent disclosures</h2>${a.items.map(x=>`<p><b>${esc(x.tool_name)}</b> · ${esc(x.decision)} · ${esc(x.created_at)}</p>`).join('')||'<p class="muted">No activity.</p>'}</section>`}catch(e){document.querySelector('#app').innerHTML=`<div class="card danger">${esc(e.message)}</div><div class="card"><h2>First-time setup</h2><p>Create a workspace when your authenticated account has not joined one yet.</p><button onclick="createWorkspace()">Create workspace</button></div>`}}load();
</script></body></html>"""
